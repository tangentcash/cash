...
RUN cmake -G Ninja -S=/home/tangentcash/source -B=/home/tangentcash/make $CONFIGURE -DTAN_TEST=ON -DCMAKE_RUNTIME_OUTPUT_DIRECTORY=/usr/local/bin -DCMAKE_LIBRARY_OUTPUT_DIRECTORY=/usr/local/lib
ARG COMPILE="-j"
...
COPY ./regtest.json /etc/tangentcash.json
RUN tangentcash --test=regression /etc/tangentcash.json
EXPOSE 18418
EXPOSE 18419
EXPOSE 18420
ENTRYPOINT ["tangentcash", "--test=consensus", "--test-account=1", "/etc/tangentcash.json"]