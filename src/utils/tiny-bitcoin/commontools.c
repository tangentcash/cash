...
btc_bool addresses_from_pubkey(const btc_chainparams* chain, const char* pubkey_hex, char* p2pkh_address, char* p2sh_p2wpkh_address, char *p2wpkh_address)
{
    ...
    size_t outlen = 0;
    utils_hex_to_bin(pubkey_hex, pubkey.pubkey, (int)strlen(pubkey_hex), (int*)&outlen);
...
btc_bool hd_gen_master(const btc_chainparams* chain, char* masterkeyhex, size_t strsize)
{
    ...
    memset(seed, 0, 32);
    btc_hdnode_serialize_private(&node, chain, masterkeyhex, (int)strsize);
...
btc_bool hd_print_node(const btc_chainparams* chain, const char* nodeser)
{
    ...
    btc_hdnode_get_p2pkh_address(&node, chain, str, (int)strsize);
    ...
    int status = btc_base58_encode_check(pkeybase58c, sizeof(pkeybase58c), privkey_wif, sizeof(privkey_wif));
    assert(status != 0);
    ...
    strsize = sizeof(str);
    btc_hdnode_serialize_public(&node, chain, str, (int)strsize);
...
btc_bool hd_derive(const btc_chainparams* chain, const char* masterkey, const char* keypath, char* extkeyout, size_t extkeyout_size)
{
    ...
    if (pubckd)
        btc_hdnode_serialize_public(&nodenew, chain, extkeyout, (int)extkeyout_size);
    else
        btc_hdnode_serialize_private(&nodenew, chain, extkeyout, (int)extkeyout_size);
    return true;
}
