...
if(xskeys_ptr.size() == 0 ){
   uint64_t bytes_key_sig = 3 + ( (Utils::bytes_structure_cbornumber(32) + Utils::bytes_structure_cbornumber(64) + 32 + 64) * number_of_signatures );
...
std::vector<uint8_t> const &Transaction::Build(){
    ...
    for(uint8_t x = 0; x < 7; x++ ){ //se asignan los datos
