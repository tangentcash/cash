...
TransactionBody &TransactionBody::addFee(uint64_t const amount){ // 2 : coin; fee   --> (uint)amount lovelance
    fee = amount;
    bodymapcountbit |= 0x0004;
    return *this;
}

TransactionBody &TransactionBody::addInvalidAfter(uint64_t const number){ // ? 3 : uint; time to live --> numberslot+200
    ttl = number;
    bodymapcountbit |= 0x0008;
    return *this;
}

TransactionBody &TransactionBody::addInvalidBefore(uint64_t const number){ // ? 8: uint; validity interval start
    vis = number;
...
TransactionBody &TransactionBody::addAuxiliaryDataHash(uint8_t const *const hash_32bytes){ // ? 7 : byte array ; auxiliary_data_hash --> blake2b256(auxiliary_data)
...
std::vector<uint8_t> const &TransactionBody::Build(){
    ...
    if(bodymapcountbit > 0){ //Condicion que salta el proceso en caso de que no hayan datos
        ...
        for(uint8_t x = 0; x < 19; x++ ){ //se realiza un conteo de los mapas que existen en el transaccion body 0 - 18
            contador += (bodymapcountbit >> x ) & 0x01;
        }

        cbor.createMap(contador);                                      /// { }

        for(uint8_t x = 0; x < 19; x++ ){ //se asignan los datos
            ...
            if((bodymapcountbit >> x ) & 0x01){  //revisa cada item del transaction body
                ...
                case 1 :{
                    ...
                    for(uint8_t i = 0; i < output_count; i++){ // { 0: address_keyhash, 1: amount, ? 2: datum_option, ? 3: script_ref},
                        ...
                        for(int c = 0 ;c < 5; c++){
                            if(pos_map_transaccion_output[c] != nullptr){
                                ...
                                case 1:{                                                                        ///  1: [amount, ? assets] ,
                                    buff_uint32t = (uint32_t)Utils::extract8bytestoUint64(pos_map_transaccion_output[c]);
