...
NativeScripts & NativeScripts::requireAllOf(){
    if(eoc != 0){
        ...
        capsula.push_back( std::vector<uint8_t>(0) );
        capsula.push_back( std::vector<uint8_t>(0) );// comienza del 1
    }
    eoc ++;
    pec = eoc;
...
NativeScripts & NativeScripts::requireAtLeastNOf(uint64_t const n){
    if(eoc != 0){
        capsula[pec][0] += 1;
        ppec.push_back(pec);
        capsula.push_back( std::vector<uint8_t>(0) );
...
NativeScripts & NativeScripts::thisIsValidBeforeSlot(uint64_t const slot){
    ...
    std::vector<uint8_t> const & vector_buffer = cbor.getCbor();
...
NativeScripts & NativeScripts::thisIsValidAfterSlot(uint64_t const slot){
    capsula[pec][0] += 1;  // Agrega +1 a la cantidad de elementos
    //CborSerialize cbor( &vector_buffer );
...
NativeScripts & NativeScripts::SignatureOf(uint8_t const *const vk_hash){
    capsula[pec][0] += 1;  // Agrega +1 a la cantidad de elementos
    //CborSerialize cbor( &vector_buffer );
    cbor.createArray(2).addUint( static_cast<uint64_t>(0) ).addBytesArray(vk_hash, 28);
    std::vector<uint8_t> const & vector_buffer = cbor.getCbor();
    capsula[pec].insert(capsula[pec].end(), vector_buffer.begin(), vector_buffer.end());  // Agrega el elemento serializado en cbor a la capsula
    //vector_buffer.clear();
    cbor.clearCbor();
...
std::vector<uint8_t> const &NativeScripts::getCborNativeScripts(){
    //CborSerialize cbor( &cbornativescript );
    cbor.clearCbor();
    std::vector<uint8_t>::iterator it = condiciones.begin();
