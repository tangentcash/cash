...
/// Vector<uint8_t> redeemer_withdrawals (redeeemer) :
...
/// Vector<uint8_t> withdrawals (withdrawals) :
/// + 2 (index)
...
Withdrawals & Withdrawals::addWithdrawals(uint8_t const * const raw_stake_address, uint64_t const amount){ // ? 5 : withdrawals
...
Withdrawals & Withdrawals::addWithdrawals(std::string & stake_address, uint64_t const amount){ // ? 5 : withdrawals
    /// 2 (index) + 29 (raw stake address)  + 8 (amount) = 39
...
void Withdrawals::alphanumeric_organization(){
    ...
    if(withdrawals_count > 0){
        ...
        for(uint16_t i = 0; i < withdrawals_count; i++ ){ // asigno las localizaciones a punteros
            ptr_wdrl[i] = ptr_data;
            ptr_data += 39;
        }
    ...
    if(redeemer_withdrawals_count > 0){
        ...
        for(uint16_t  i= 0; i<redeemer_withdrawals_count;i++ ){ // asigno las localizaciones a punteros
