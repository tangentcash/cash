...
bool const pbkdf2_hmac512_libsodium( uint8_t const * const key, std::size_t key_len,
                               uint8_t const * const salt, std::size_t salt_len,
                               uint64_t const iterations,
                               std::size_t out_len , uint8_t * const out ) noexcept {
    ...
    for (uint8_t i = 0; i < T_max; i++) {                            // (Ciclo Bloques) ; T_(i)
