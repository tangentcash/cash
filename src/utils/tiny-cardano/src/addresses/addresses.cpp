...
static void set_script(uint8_t const * const input_script, std::size_t const * const input_script_len, ScriptType const script_type,
                       uint8_t * const output_scripthash){
...
static void set_address(Address const * const address_type, uint8_t const * const input_key, InputKey const input_key_type, Wallet const wallet_type,
                        uint32_t const * const account_path, uint32_t const * const address_index_path, uint8_t * const header,
                        uint8_t * const payload_left, uint8_t *const payload_right,
                        uint8_t * const buff_xvk){
...
void getBech32Address( Cardano::InputKey const input_key_type, uint8_t const * const input_key, Cardano::Network const network_id, Cardano::Wallet const wallet_type, Cardano::Address const address_type,
                       uint32_t const account_path, uint32_t const address_index_path,
...
void getRawAddress( Cardano::InputKey const input_key_type, uint8_t const * const input_key, Cardano::Network const network_id, Cardano::Wallet const wallet_type, Cardano::Address const address_type,
                    uint32_t const account_path, uint32_t const address_index_path,
                    uint8_t * const output_raw,uint8_t * const output_raw_len){
...
void getRawAddressKeyHash( Cardano::InputKey const input_key_type, uint8_t const * const input_key,
                           uint32_t const account_path,uint8_t * const addresskeyhash_output, uint8_t * const addresskeyhash_len ){
...
void getBech32ScriptHash( Cardano::ScriptType const script_type, std::string const &script, std::string & address_output ){
    ...
    uint8_t const * const script_uint8_t = (script_type != ScriptType::None)?Utils::hexchararray2uint8array(script, &script_uint8_t_len):nullptr;
...
void getRawScriptHash( Cardano::ScriptType const script_type, std::string const &script,
                       uint8_t * const scripthash_output, uint8_t * const scripthash_len ){
