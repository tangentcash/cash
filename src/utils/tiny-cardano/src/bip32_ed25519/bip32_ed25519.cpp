...
bool valid_ed25519_sk( uint8_t const * const raw_privatekey_sk ) noexcept { //si en a[0] sus tres bit mas bajos no son cero y a[31] su bit mas alto no es cero entonces la llave no es valida
...
bool rawprivatekey_to_rawpublickey( uint8_t const * const raw_privatekey_xsk, uint8_t * const raw_publickey_xvk ) noexcept{
    ...
    for(uint8_t i = 64 ; i < 96; i++){  //Copia el chain code del privatekey
...
bool signature_deprecated( uint8_t const * const raw_privatekey_xsk, uint8_t const * const message,std::size_t const message_len, uint8_t * const out ) noexcept {
    ...
    for(uint8_t i = 32; i<64; i++){      // public key[0..32] -> out[32..64]
