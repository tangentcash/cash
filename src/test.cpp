...
struct generators
{
	...
	static void setup_stage_0(vector<uptr<ledger::transaction_message>>& transactions, vector<account_ref>& users)
	{
		...
		setup_user1->allocate_bridge(algorithm::asset::id_of("ETH"), (uint8_t)protocol::now().policy.participation.min_per_account, 0.001);
		setup_user1->allocate_bridge(algorithm::asset::id_of("TRX"), (uint8_t)protocol::now().policy.participation.min_per_account, 20);
		setup_user1->allocate_bridge(algorithm::asset::id_of("BTC"), (uint8_t)protocol::now().policy.participation.min_per_account, 0.0005);
	...
	static void setup_stage_1(vector<uptr<ledger::transaction_message>>& transactions, vector<account_ref>& users)
	{
		...
		setup_user1->allocate_bridge(algorithm::asset::id_of("XRP"), (uint8_t)protocol::now().policy.participation.min_per_account, 0.001);
		setup_user1->sign(user1.secret_key, user1_nonce++, decimal::zero()).expect("pre-validation failed");
		transactions.push_back(setup_user1);
		...
		setup_user2->allocate_bridge(algorithm::asset::id_of("ETH"), (uint8_t)protocol::now().policy.participation.min_per_account, 0.0012);
		setup_user2->allocate_bridge(algorithm::asset::id_of("XRP"), (uint8_t)protocol::now().policy.participation.min_per_account, 1.0);
		setup_user2->allocate_bridge(algorithm::asset::id_of("BTC"), (uint8_t)protocol::now().policy.participation.min_per_account, 0.000025);
	...
	static void migrate_stage_2(vector<uptr<ledger::transaction_message>>& transactions, vector<account_ref>& users)
	{
		...
		size_t users_ending_count = users_starting_count + protocol::now().policy.participation.max_per_account;
...
struct tests
{
	...
	static void blockchain_full_coverage(vector<account_ref>* userdata)
	{
		tester::use_clean_state([&]()
		{
			...
			TEST_BLOCK(&generators::setup_stage_1, "0x299692e9cd8a9ce7754cb1ebe629a674f3edc218cd1347bf506d227f0d43b52e", 1);
			TEST_BLOCK(std::bind(&generators::setup_custom, std::placeholders::_1, std::placeholders::_2, 2, 1, 0), "0x910dc5571a01e7fc3a101781d4ee44e6cd66ffe96f9387268a80952dac2d61f9", 2);
			TEST_BLOCK(&generators::route_stage_1, "0xb51457455cbc2ffd18cee1a30ea89c9d9db4adc84191d8d7fceae0877c2d614c", 3);
			TEST_BLOCK(&generators::route_stage_2, "0xa2eb4a65f28979baf710b8ed249e44a7e27125305f284d6f1c2bc48d98069441", 5);
			TEST_BLOCK(&generators::attestate_stage_1, "0x9b95c7d9b9be925a8b2d1bd0803de84df38967019541a7580a9455c27870c290", 7);
			TEST_BLOCK(&generators::transfer_stage_1, "0x4b91328255b8b7ee83b43530553355f9a7765fdf28fd8af5b641725bbe7af38d", 8);
			TEST_BLOCK(&generators::transfer_stage_2, "0xa872b1efc2a79d2289d24341958fce9e87992a00960df7d33e938839a5e475ad", 9);
			TEST_BLOCK(std::bind(&generators::transfer_custom, std::placeholders::_1, std::placeholders::_2, 0, algorithm::asset::id_of("BTC"), users[2].wallet.get_address(), 0.05), "0x52a9dbed71610f38bce8fe284b41603733fa63f95d80bc634739335d6b47599c", 10);
			TEST_BLOCK(std::bind(&generators::deploy_stage_1, std::placeholders::_1, std::placeholders::_2, &contracts), "0xe5ebed7c6a57fb75fe1e17db730e892064ac38e17067f34fbaf17cea9a263984", 11);
			TEST_BLOCK(std::bind(&generators::deploy_stage_2, std::placeholders::_1, std::placeholders::_2, &contracts), "0xc01495fc89c81bd6e8d85e38a264ee9b41fe9a90cbdff15b1edcc48c87e18fea", 12);
			TEST_BLOCK(std::bind(&generators::call_stage_1, std::placeholders::_1, std::placeholders::_2, &contracts), "0xaa7562308dd1b7b053260f8eab7b26a80b83c5fd0fbc2a7e2802c563a28f8bf3", 13);
			TEST_BLOCK(&generators::rollup_stage_1, "0x01c60bd827a61815ceb7e7a6e550413ca9c47e39000f8a94bf5f65080bdb61d2", 14);
			TEST_BLOCK(std::bind(&generators::setup_custom, std::placeholders::_1, std::placeholders::_2, 2, 0, 1), "0xc3a114e930b269614ba0762bf846fba1fdb87473a01edb04aeac9315c5cb71ec", 15);
			TEST_BLOCK_FAULTY(&generators::migrate_stage_1, "0xbf6a66ebc63d746edfee0e625e6124b4763ad16beb03b0ba0682603d96d1b01a", 16);
			TEST_BLOCK(&generators::migrate_stage_2, "0x5188257c7c307d6e12da717c874e4cc5c3f820c605105dd56df7c3aec76e7165", 18);
			TEST_BLOCK(&generators::migrate_stage_3, "0x3d6d5c9d7f0ea6a518efee31bacb010d9ca2731b0d641e1e4834988143432d5e", 19);
			TEST_BLOCK(&generators::migrate_stage_4, "0xe225ba6a33c034de56838d3b35e4281eff80e2d3e49cd9c1db78eb38679edef1", 21);
			TEST_BLOCK(&generators::withdraw_stage_1, "0xdacce62d66761d3b52a7a8764c3f5afc1ade707356f8f96c2c6412007ec1e626", 23);
			TEST_BLOCK(&generators::withdraw_stage_2, "0xa8dbbb202743b06d555c96c51395c5dee5df8feedce99d56731881a2cbeff11e", 25);
			TEST_BLOCK(&generators::withdraw_stage_3, "0x16d074b92c460744b14d263d8645f952f86e8dea8f9c87ae4572224659673c0e", 26);
			TEST_BLOCK(&generators::withdraw_stage_4, "0xbeab3afdacb14e03c5daae1bb5603bd6ae4a984fa89987764cbd1727eb399f57", 27);
			TEST_BLOCK(&generators::withdraw_stage_5, "0xfd2d41f770edcaa85fd1745702576dbec8095188398fd934aac4370574208a1d", 28);
			TEST_BLOCK(&generators::withdraw_stage_6, "0x7cf29ae18a73be642e39548404222389b56c69e14ca343f4f7bce0781e141d1c", 30);
			TEST_BLOCK(&generators::withdraw_stage_7, "0x1885b7efe3bf1da94458a1a2b89f3891d1bd42f9901dadadded32282166e53bc", 32);
			TEST_BLOCK(std::bind(&generators::setup_custom, std::placeholders::_1, std::placeholders::_2, 2, 1, 0), "0x4b4ecef728d4ed6db72536f982d3f427aed166096c460da5d1ca3f7355ecf14c", 34);
			TEST_BLOCK_FALLBACK(&generators::production_stage_1, "0xecaf7b3f29cb62430a63ebf6e98661934b56a0c07f8d016b817611d8157074af", 35);
			TEST_BLOCK(&generators::production_stage_2, "0xe7d20463eb78408b7818015472bc432b5b82ed3ecc8b8ef8627d3a54df431c6c", 36);
			if (userdata != nullptr)
	...
	static void blockchain_partial_coverage(vector<account_ref>* userdata)
	{
		tester::use_clean_state([&]()
		{
			...
			format::tree results;
			format::tree* data = userdata ? nullptr : &results;
			TEST_BLOCK(&generators::setup_stage_0, "0x60cf96c18d26503363e547f52b0b7bb62f3e10b3cf0411f386d2c74ef77a747e", 1);
			TEST_BLOCK(&generators::route_stage_0, "0x2feeb11773a166e98e7ffe284e608091b9760704c904fac58bd0689b6a1554f2", 2);
			TEST_BLOCK(&generators::attestate_stage_0, "0x7a42c01327e678515a405994d6d9a5033192c4cf6985ad5b74cc2c3fbfbca4d6", 4);
			TEST_BLOCK(std::bind(&generators::transfer_custom, std::placeholders::_1, std::placeholders::_2, 0, algorithm::asset::id_of("BTC"), "tcrt1x00g22stp0qcprrxra7x2pz2au33armtfc50460", 5), "0x2b89b6a9bb1ac28f3e977a2467da5793ebdc0c3907ac6c4d25a27a2ae9dcced3", 5);
			TEST_BLOCK(std::bind(&generators::transfer_custom, std::placeholders::_1, std::placeholders::_2, 0, algorithm::asset::id_of("ETH", "tBTC", "0x18084fbA666a33d37592fA2633fD49a74DD93a88"), "tcrt1x00g22stp0qcprrxra7x2pz2au33armtfc50460", 5), "0x03123714c49b276f0abb9ce918f974c21c18a613ec5c96126dd9751f7343f5cd", 6);
			TEST_BLOCK(std::bind(&generators::transfer_custom, std::placeholders::_1, std::placeholders::_2, 0, algorithm::asset::id_of("ETH", "USDT", "0xdAC17F958D2ee523a2206206994597C13D831ec7"), "tcrt1x00g22stp0qcprrxra7x2pz2au33armtfc50460", 300000), "0x63f0c335c250ffe3e9055a751d8eabba6afbbf40d26f451233eba3eeec7c2dcf", 7);
			TEST_BLOCK(std::bind(&generators::transfer_custom, std::placeholders::_1, std::placeholders::_2, 0, algorithm::asset::id_of("TRX", "USDT", "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t"), "tcrt1x00g22stp0qcprrxra7x2pz2au33armtfc50460", 200000), "0x3f50d980fc2bed271d57fa02558ad32488825126684097c1105583c94849c9e1", 8);
			TEST_BLOCK(std::bind(&generators::transfer_custom, std::placeholders::_1, std::placeholders::_2, 0, algorithm::asset::id_of("BTC"), "tcrt1xu0k7jd2hsv2x5h80tcslpk3n0kvzzw5kup6vng", 5), "0x169eb31953c0e65c6310de239ecb5b8c917b70d39b9f411c5da03760b2d9a8d8", 9);
			TEST_BLOCK(std::bind(&generators::transfer_custom, std::placeholders::_1, std::placeholders::_2, 0, algorithm::asset::id_of("ETH", "tBTC", "0x18084fbA666a33d37592fA2633fD49a74DD93a88"), "tcrt1xu0k7jd2hsv2x5h80tcslpk3n0kvzzw5kup6vng", 5), "0xefe868d6f7e51c5fa711e9ada52b2e82f27b01a2667adca895dce9cf0ddecb3e", 10);
			TEST_BLOCK(std::bind(&generators::transfer_custom, std::placeholders::_1, std::placeholders::_2, 0, algorithm::asset::id_of("ETH", "USDT", "0xdAC17F958D2ee523a2206206994597C13D831ec7"), "tcrt1xu0k7jd2hsv2x5h80tcslpk3n0kvzzw5kup6vng", 300000), "0x7f266435ef20bbbbc77f32b1048e8fdf7a4c145d67552843e5f48bb21245ead0", 11);
			TEST_BLOCK(std::bind(&generators::transfer_custom, std::placeholders::_1, std::placeholders::_2, 0, algorithm::asset::id_of("TRX", "USDT", "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t"), "tcrt1xu0k7jd2hsv2x5h80tcslpk3n0kvzzw5kup6vng", 200000), "0x2989151c82c12cc0e431fdc034f4d117eb71f33ad5073e8b0ae9eecf2bbe2629", 12);
			if (userdata != nullptr)
				*userdata = std::move(users);
			else
				console::get()->write_line(data->as_json(true));
