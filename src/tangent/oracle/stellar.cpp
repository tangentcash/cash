...
namespace Tangent
{
	namespace Oracle
	{
		namespace Chains
		{
			...
			Stellar::Stellar(ChainConfig* NewConfig) noexcept : Chainmaster()
			{
			...
			Promise<ExpectsLR<Stellar::AssetInfo>> Stellar::GetAssetInfo(const Algorithm::AssetId& Asset, const std::string_view& Address, const std::string_view& Code)
			{
				...
				if (!AssetData)
					Coreturn ExpectsLR<Stellar::AssetInfo>(std::move(AssetData.Error()));

				UPtr<Schema> AssetWrap = *AssetData;
				Schema* Records = AssetWrap->Fetch("_embedded.records");
				if (!Records)
					Coreturn ExpectsLR<Stellar::AssetInfo>(LayerException("contract address not found"));
				...
				for (auto& Asset : Records->GetChilds())
				{
					...
					if (Info.Issuer == Address)
						Coreturn ExpectsLR<Stellar::AssetInfo>(std::move(Info));
				}

				Coreturn ExpectsLR<Stellar::AssetInfo>(LayerException("contract address not found"));
			}
			Promise<ExpectsLR<Stellar::AccountInfo>> Stellar::GetAccountInfo(const Algorithm::AssetId& Asset, const std::string_view& Address)
			{
				auto AccountData = Coawait(ExecuteREST(Asset, "GET", NdCall::GetAccounts(Address), nullptr, CachePolicy::Lazy));
				if (!AccountData)
					Coreturn ExpectsLR<Stellar::AccountInfo>(std::move(AccountData.Error()));
				...
				Coreturn ExpectsLR<Stellar::AccountInfo>(std::move(Info));
			}
			Promise<ExpectsLR<String>> Stellar::GetTransactionMemo(const Algorithm::AssetId& Asset, const std::string_view& TxId)
			{
				...
				if (!TxData)
					Coreturn ExpectsLR<String>(std::move(TxData.Error()));

				String Memo = TxData->GetVar("memo").GetBlob();
				if (Memo.empty())
					Coreturn ExpectsLR<String>(LayerException("transaction memo not found"));

				Coreturn ExpectsLR<String>(std::move(Memo));
			}
			Promise<bool> Stellar::IsAccountExists(const Algorithm::AssetId& Asset, const std::string_view& Address)
			{
				auto AccountData = UPtr<Schema>(Coawait(ExecuteREST(Asset, "GET", NdCall::GetAccounts(Address), nullptr, CachePolicy::Lazy)));
				Coreturn AccountData&& AccountData->Has("account_id");
			}
			Promise<ExpectsLR<void>> Stellar::BroadcastTransaction(const Algorithm::AssetId& Asset, const OutgoingTransaction& TxData)
			{
				...
				if (!Detail.empty())
				{
					...
					Coreturn ExpectsLR<void>(LayerException(std::move(Code.empty() ? Detail : Code)));
				}

				Memory::Release(*HexData);
				Coreturn ExpectsLR<void>(Expectation::Met);
			}
			Promise<ExpectsLR<uint64_t>> Stellar::GetLatestBlockHeight(const Algorithm::AssetId& Asset)
			{
				auto LastBlockData = Coawait(ExecuteREST(Asset, "GET", NdCall::GetLastLedger(), nullptr, CachePolicy::Lazy));
				if (!LastBlockData)
					Coreturn ExpectsLR<uint64_t>(std::move(LastBlockData.Error()));
			...
			Promise<ExpectsLR<Schema*>> Stellar::GetBlockTransactions(const Algorithm::AssetId& Asset, uint64_t BlockHeight, String* BlockHash)
			{
				auto BlockData = Coawait(ExecuteREST(Asset, "GET", NdCall::GetLedgerOperations(BlockHeight), nullptr, CachePolicy::Shortened));
				if (!BlockData)
					Coreturn ExpectsLR<Schema*>(std::move(BlockData.Error()));
				...
				if (!Data)
				{
					...
					Coreturn ExpectsLR<Schema*>(LayerException("block not found"));
				}
			...
			Promise<ExpectsLR<Schema*>> Stellar::GetBlockTransaction(const Algorithm::AssetId& Asset, uint64_t BlockHeight, const std::string_view& BlockHash, const std::string_view& TransactionId)
			{
			...
			Promise<ExpectsLR<Vector<IncomingTransaction>>> Stellar::GetAuthenticTransactions(const Algorithm::AssetId& Asset, uint64_t BlockHeight, const std::string_view& BlockHash, Schema* TransactionData)
			{
				auto* Implementation = (Chains::Stellar*)Datamaster::GetChain(Asset);
				if (!Implementation)
			...
			Promise<ExpectsLR<BaseFee>> Stellar::EstimateFee(const Algorithm::AssetId& Asset, const DynamicWallet& Wallet, const Vector<Transferer>& To, const FeeSupervisorOptions& Options)
			{
				auto* Implementation = (Chains::Stellar*)Datamaster::GetChain(Asset);
				if (Algorithm::Asset::TokenOf(Asset).empty())
					Coreturn ExpectsLR<BaseFee>(BaseFee(Implementation->FromStroop(Implementation->GetBaseStroopFee() * To.size()), Decimal(1.0)));

				uint64_t Fee = Implementation->GetBaseStroopFee() * To.size();
				for (auto& Item : To)
				{
					if (!Coawait(IsAccountExists(Asset, Item.Address)))
						Fee += Implementation->GetBaseStroopFee();
				}

				Coreturn ExpectsLR<BaseFee>(BaseFee(Implementation->FromStroop(Fee), Decimal(1.0)));
			}
			Promise<ExpectsLR<Decimal>> Stellar::CalculateBalance(const Algorithm::AssetId& Asset, const DynamicWallet& Wallet, Option<String>&& Address)
			{
				auto* Implementation = (Chains::Stellar*)Datamaster::GetChain(Asset);
				if (!Address)
				{
				...
				if (Balance == Account->Balances.end())
					Coreturn ExpectsLR<Decimal>(Decimal::Zero());

				auto ContractAddress = Datamaster::GetContractAddress(Asset);
				if (ContractAddress && Balance->second.Info.Issuer != *ContractAddress)
					Coreturn ExpectsLR<Decimal>(Decimal::Zero());

				Coreturn ExpectsLR<Decimal>(Balance->second.Balance);
			}
			Promise<ExpectsLR<OutgoingTransaction>> Stellar::NewTransaction(const Algorithm::AssetId& Asset, const DynamicWallet& Wallet, const Vector<Transferer>& To, const BaseFee& Fee)
			{
				...
				for (auto& Item : To)
				{
					if (Item.AddressIndex)
					{
						if (Memo.empty())
							...
							Coreturn ExpectsLR<OutgoingTransaction>(LayerException("input memo invalid"));
					}
					else if (!Memo.empty())
						Coreturn ExpectsLR<OutgoingTransaction>(LayerException("input memo invalid"));
				}
				...
				if (ContractAddress)
				{
					...
					if (Native == AccountInfo->Balances.end() || Native->second.Balance < FeeValue)
						Coreturn ExpectsLR<OutgoingTransaction>(LayerException(Stringify::Text("insufficient funds: %s < %s", (Native != AccountInfo->Balances.end() ? Native->second.Balance : Decimal(0.0)).ToString().c_str(), FeeValue.ToString().c_str())));
				}
				else
					TotalValue += FeeValue;
				...
				if (Token == AccountInfo->Balances.end() || Token->second.Balance < TotalValue)
					Coreturn ExpectsLR<OutgoingTransaction>(LayerException(Stringify::Text("insufficient funds: %s < %s", (Token != AccountInfo->Balances.end() ? Token->second.Balance : Decimal(0.0)).ToString().c_str(), FeeValue.ToString().c_str())));
				...
				if (ContractAddress)
				{
					...
					if (Token->second.Info.Type == "credit_alphanum4")
						...
						Coreturn ExpectsLR<OutgoingTransaction>(LayerException("standard not supported"));
				...
				uint8_t Signature[crypto_sign_BYTES];
				{
					...
					if (!DecodePrivateKey(FromWallet->SigningKey.ExposeToHeap(), DerivedPrivateKey))
						Coreturn ExpectsLR<OutgoingTransaction>(LayerException("private key invalid"));
