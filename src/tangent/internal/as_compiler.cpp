	...
	else if (vnode->nodeType == snConstant)
	{
				 ...
				 vnode->tokenType == ttFalse)
		{
			...
			ctx->type.SetConstantDW(asCDataType::CreatePrimitive(ttBool, true), vnode->tokenType == ttTrue ? VALUE_OF_BOOLEAN_TRUE : 0);
				 ...
				 vnode->tokenType == ttFloatConstant ||
				 vnode->tokenType == ttDoubleConstant)
		{
		asStringConstant:
			asCString str;
			asCScriptNode* snode = vnode->firstChild;
			if (vnode->tokenType == ttIntConstant || vnode->tokenType == ttBitsConstant || vnode->tokenType == ttFloatConstant || vnode->tokenType == ttDoubleConstant)
				snode = vnode;
			...
			else
			{
				...
				while (snode)
				{
					...
					else if (snode->tokenType == ttIntConstant || snode->tokenType == ttBitsConstant || snode->tokenType == ttFloatConstant || snode->tokenType == ttDoubleConstant)
					{
						cat.Assign(&script->code[snode->tokenPos], snode->tokenLength);
						ProcessStringConstant(cat, snode);
					}
