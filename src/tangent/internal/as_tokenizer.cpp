...
{
	static sTokenWord const deletedTokenWords[] =
	{
		asTokenDef("shared"  , ttUnrecognizedToken),
		asTokenDef("external", ttUnrecognizedToken),
	};
	static const unsigned int numDeletedTokenWords = sizeof(deletedTokenWords) / sizeof(sTokenWord);
	FreeJumpTable();

	// Initialize the jump table
	for (asUINT n = 0; n < numTokenWords + numDeletedTokenWords; n++)
	{
		const sTokenWord& current = n >= numTokenWords ? deletedTokenWords[n - numTokenWords] : tokenWords[n];
