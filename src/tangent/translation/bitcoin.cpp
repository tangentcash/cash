...
namespace tangent
{
	namespace superchain
	{
		namespace translations
		{
			...
			static bool cash_address_from_legacy_hash(const sc_chainparams_* chain, const uint8_t* address_hash, size_t address_hash_size, char* out_address, size_t out_address_size)
			{
			...
			static bool bitcoin_cash_public_key_get_address_p2pkh(const btc_pubkey* public_key, const sc_chainparams_* chain, char* address_out, size_t address_out_size)
			{
			...
			static bool bitcoin_cash_public_key_get_address_p2sh(const btc_pubkey* public_key, const sc_chainparams_* chain, char* address_out, size_t address_out_size)
			{
				if (chain->bech32_cashaddr[0] == '\0')
					return false;
			...
			expects_lr<string> bitcoin::encode_address(const std::string_view& public_key_hash)
			{
				...
				switch (type)
				{
					...
					case 0xC:
					{
						...
						char encoded_address[128];
						bech32_address_encode(encoded_address, chain->bech32_hrp, 0, (uint8_t*)data.data(), data.size());
						return string(encoded_address, strnlen(encoded_address, sizeof(encoded_address)));
					}
					case 0xB:
					{
					...
					case 0xA:
					{
						...
						char encoded_address[128];
						bech32_address_encode(encoded_address, chain->bech32_hrp, 1, (uint8_t*)data.data(), data.size());
						return string(encoded_address, strnlen(encoded_address, sizeof(encoded_address)));
					}
					case 0x9:
					{
					...
					case 0x6:
					{
						...
						bech32_zaddress_encode(encoded_address, chain->bech32_hrp, (uint8_t*)data.data(), data.size());
						return string(encoded_address, strnlen(encoded_address, sizeof(encoded_address)));
					}
					default:
			...
			bitcoin::address_format bitcoin::parse_address(const std::string_view& address, uint8_t* data_out, size_t* data_size_out)
			{
				...
				if (chain->bech32_hrp[0] == '\0' || stringify::starts_with(address, chain->bech32_hrp))
				{
					...
					else if (bech32_zaddress_decode(data, &data_size, chain->bech32_hrp, address_data.c_str()))
					{
