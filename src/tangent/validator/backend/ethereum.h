...
namespace Tangent
{
	namespace Mediator
	{
		namespace Backends
		{
			class Ethereum : public RelayBackend
			{
			public:
			...
			public:
				virtual ExpectsPromiseRT<Schema*> GetTransactionReceipt(const Algorithm::AssetId& Asset, const std::string_view& TxId);
				virtual ExpectsPromiseRT<uint256_t> GetTransactionsCount(const Algorithm::AssetId& Asset, const std::string_view& Address);
				virtual ExpectsPromiseRT<uint256_t> GetChainId(const Algorithm::AssetId& Asset);
				virtual ExpectsPromiseRT<String> GetContractSymbol(const Algorithm::AssetId& Asset, Backends::Ethereum* Implementation, const std::string_view& ContractAddress);
				virtual ExpectsPromiseRT<Decimal> GetContractDivisibility(const Algorithm::AssetId& Asset, Backends::Ethereum* Implementation, const std::string_view& ContractAddress);
