...
namespace Tangent
{
	namespace P2P
	{
		...
		{
			if (NSS::ServerNode::HasInstance())
			{
				auto NodeId = Codec::HexEncode(std::string_view((char*)this, sizeof(this)));
				NSS::ServerNode::Get()->AddTransactionCallback(NodeId, nullptr);
			}
			ClearPendingTip();
		}
				...
				for (auto& Seeder : Lists)
				{
					...
					if (Response)
					{
						...
						if (Addresses)
						{
							...
							{
								auto Endpoint = SystemEndpoint(Address->Value.GetBlob());
		...
		Promise<void> ServerNode::ProposeTransactionLogs(const Mediator::ChainSupervisorOptions& Options, Mediator::TransactionLogs&& Logs)
		{
		...
		Promise<void> ServerNode::VerifyForkCollision(ServerNode* Relayer, UPtr<Relay>&& From, Format::Variables&& Args)
		{
			...
			for (size_t i = 3; i < Args.size() + 1; i++)
			{
				...
				if (!Verification)
					return ReturnAbort(Relayer, *From, __func__, "invalid fork block header: " + Verification.Error().message());
		...
		void ServerNode::Startup()
		{
			...
			Validator.Node.Services.Synchronization = Protocol::Now().User.NSS.Server;
				...
				if (Mempool.ActivationBlock && (!*Mempool.ActivationBlock || *Mempool.ActivationBlock <= Storages::Chainstate(__func__).GetLatestBlockNumber().Or(0)))
				{
					...
					if (!Priority)
					{
						...
						if (Tip)
						{
							...
							if (Delta < 0 || (uint64_t)Delta < Protocol::Now().Policy.ConsensusRecoveryTime)
							{
