...
namespace tangent
{
	namespace oracle
	{
		...
		struct transaction_listener
		{
			...
			warden::chain_supervisor_options options;
		...
		protected:
			...
			unordered_map<string, std::pair<warden::computed_fee, int64_t>> fees;
			unordered_map<string, vector<uptr<warden::server_relay>>> nodes;
			unordered_map<string, uptr<warden::relay_backend>> chains;
			unordered_map<string, uptr<schema>> specifications;
			vector<uptr<transaction_listener>> listeners;
			warden::multichain_supervisor_options options;
		...
		public:
			...
			expects_promise_rt<schema*> execute_rpc(const algorithm::asset_id& asset, const std::string_view& method, schema_list&& args, warden::cache_policy cache);
			...
			expects_promise_rt<warden::transaction_logs> link_transactions(const algorithm::asset_id& asset, warden::chain_supervisor_options* options);
			expects_promise_rt<warden::computed_transaction> link_transaction(const algorithm::asset_id& asset, uint64_t block_height, const std::string_view& block_hash, schema* transaction_data);
			...
			expects_promise_rt<warden::prepared_transaction> prepare_transaction(const algorithm::asset_id& asset, const warden::wallet_link& from_link, const vector<warden::value_transfer>& to, option<warden::computed_fee>&& fee = optional::none);
			...
			expects_lr<void> enable_link(const algorithm::asset_id& asset, const warden::wallet_link& link);
			expects_lr<void> disable_link(const algorithm::asset_id& asset, const warden::wallet_link& link);
			expects_lr<warden::wallet_link> normalize_link(const algorithm::asset_id& asset, const warden::wallet_link& link);
			...
			expects_lr<vector<warden::coin_utxo>> get_utxos(const algorithm::asset_id& asset, const warden::wallet_link& link, size_t offset, size_t count);
			expects_lr<schema*> load_cache(const algorithm::asset_id& asset, warden::cache_policy policy, const std::string_view& key);
			expects_lr<void> store_cache(const algorithm::asset_id& asset, warden::cache_policy policy, const std::string_view& key, uptr<schema>&& value);
			option<string> get_contract_address(const algorithm::asset_id& asset);
			unordered_map<algorithm::asset_id, warden::relay_backend::chainparams> get_chains();
			unordered_map<string, invocation_callback>& get_registrations();
			vector<algorithm::asset_id> get_assets(bool observing_only = false);
			vector<uptr<warden::server_relay>>* get_nodes(const algorithm::asset_id& asset);
			const warden::relay_backend::chainparams* get_chainparams(const algorithm::asset_id& asset);
			warden::server_relay* add_node(const algorithm::asset_id& asset, const std::string_view& url, double rps);
			warden::server_relay* add_multi_node(const algorithm::asset_id& asset, unordered_map<string, string>&& urls, double rps);
			warden::server_relay* get_node(const algorithm::asset_id& asset);
			warden::relay_backend* get_chain(const algorithm::asset_id& asset);
			...
			warden::multichain_supervisor_options& get_options();
			...
			bool has_warden(const algorithm::asset_id& asset);
		...
		private:
			void add_node_instance(const algorithm::asset_id& asset, warden::server_relay* instance);
			void add_chain_instance(const algorithm::asset_id& asset, warden::relay_backend* instance);
