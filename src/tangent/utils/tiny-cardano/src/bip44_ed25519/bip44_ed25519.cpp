...
static bool from_masterkey(uint8_t const * const extended_master_secret_key, Wallet const * const wallet_type, OutputKey const * const output_key_type, Role const * const role_path,
                           uint32_t const * const index_acc, int * const keysize, uint32_t const * const address_index_path,
                           uint8_t *const buff_xsk) noexcept {
    ...
    case OutputKey::Private:{
        *keysize = XSK_LENGTH;
        if(*role_path != Role::OnlyAccount){
            ...
            case Wallet::HD:{
                if(!raw_child_privatekey(buff_xsk, static_cast<uint32_t>(*role_path), buff_xsk)){      /// m/1852'/1815'/account'/role_path
                    return false;
                }
            };break;
    ...
    case OutputKey::Public:{
        *keysize = XVK_LENGTH;
        ...
        if(*role_path != Role::OnlyAccount){
            ...
            case Wallet::HD:{
                if(!raw_child_publickey(buff_xsk, static_cast<uint32_t>(*role_path), buff_xsk)){       /// m/1852'/1815'/account'/role_path
                    return false;
                }
            };break;
...
static bool from_accountkey(uint8_t const * const account_key, InputKey const * const account_key_type, Wallet const * const wallet_type, OutputKey const * const output_key_type, Role const * const role_path,
                            int * const keysize, uint32_t const * const address_index_path,
                            uint8_t * const buff_xsk) noexcept {
        ...
        if(*account_key_type == InputKey::AccountKey_xsk){
            *keysize = XSK_LENGTH;
            if(*role_path != Role::OnlyAccount){
                ...
                case Wallet::HD:{
                    if(!raw_child_privatekey(account_key, static_cast<uint32_t>(*role_path), buff_xsk)){  /// m/1852'/1815'/account'/role_path
                        return false;
                    }
                };break;
...
bool getRawKey( Cardano::InputKey input_key_type, uint8_t const * const input_key, Cardano::Wallet wallet_type, Cardano::OutputKey output_key_type,
                    uint32_t const account_path, Cardano::Role role_path, uint32_t const address_index_path,
                    uint8_t * const output_key) noexcept {
...
bool getRawKey( Cardano::InputKey input_key_type, uint8_t const * const input_key, Cardano::Wallet wallet_type, Cardano::OutputKey output_key_type,
                   uint32_t const account_path, Cardano::Role role_path, uint8_t * const output_key) noexcept {
...
bool getBech32key( Cardano::InputKey input_key_type, uint8_t const * const input_key, Cardano::Wallet wallet_type, Cardano::OutputKey output_key_type,
                    uint32_t const account_path, Cardano::Role role_path, uint32_t const address_index_path,
...
//for generate only account
bool getBech32key( Cardano::InputKey input_key_type, uint8_t const * const input_key, Cardano::Wallet wallet_type, Cardano::OutputKey output_key_type,
               uint32_t const account_path, Cardano::Role role_path, std::string & bech32_output_key) noexcept{
