...
/// Vector<uint8_t> tx_input / reference_input / collateral_input:
/// +2  (index) // indica el orden
/// +32 (TxHash)
/// +8  (TxIx)
/// 42 bytes maximo de largo minimo

/// Vector<uint8_t> datum_input (datum) :
...
/// Vector<uint8_t> redeemer_input (redeeemer) :
...
bool TransactionsInputs::addUtxoInput(uint8_t const t_selector, uint8_t const * const & TxHash,uint64_t const & TxIx){   //0 : set<transaction_input> --> transaction_input = [ transaction_id : hash32, index : uint]
...
TransactionsInputs &TransactionsInputs::addInput(std::string const &TxHash, uint64_t const TxIx){
    ...
    uint8_t const * const TxHash_uint8t = Utils::hexchararray2uint8array(TxHash, &txhash_len);
    if(txhash_len == 32){
...
TransactionsInputs &TransactionsInputs::addInlineScript(Cardano::ScriptType const script_type, std::string const & TxHash, uint64_t const TxIx){
...
TransactionsInputs &TransactionsInputs::addScript(Cardano::ScriptType const script_type, uint8_t const * const & script, std::size_t & script_len){
    switch(script_type){
...
TransactionsInputs & TransactionsInputs::addScript( Cardano::ScriptType const script_type, std::string const & script ){
    ...
    uint8_t const *script_tipo = Utils::hexchararray2uint8array(script, &script_tipo_len);
...
void TransactionsInputs::alphanumeric_organization(){
    ...
    if(tx_input_count > 0){
        ...
        for(uint16_t  i = 0; i < tx_input_count; i++ ){ // asigno las localizaciones a punteros
    ...
    if(reference_input_count > 0){
        ...
        for(uint16_t  i= 0; i<reference_input_count;i++ ){ // asigno las localizaciones a punteros
    ...
    if(collateral_input_count > 0){
        ...
        for(uint16_t  i= 0; i<collateral_input_count;i++ ){ // asigno las localizaciones a punteros
    ...
    if(datum_input_count > 0){
        ...
        for(uint16_t  i= 0; i<datum_input_count;i++ ){ // asigno las localizaciones a punteros
    ...
    if(redeemer_input_count > 0){
        ...
        for(uint16_t  i= 0; i<redeemer_input_count;i++ ){ // asigno las localizaciones a punteros
