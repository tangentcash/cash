...
TransactionsOutputs &TransactionsOutputs::addAsset(std::string  policyID, std::string assetname, uint64_t const amount){
    ...
    uint8_t const *policy_id =  Utils::hexchararray2uint8array(policyID, &policy_id_len);
    if(policy_id_len != 28){
            delete[] policy_id;
...
TransactionsOutputs &TransactionsOutputs::addInlineScript(Cardano::ScriptType const script_type, uint8_t const *const script_, std::size_t &script_len ){
...
uint8_t const *script_tipo = Utils::hexchararray2uint8array(script_, &script_tipo_len);
